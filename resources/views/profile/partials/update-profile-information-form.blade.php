    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <x-text-input id="session" name="session" type="text" class="mt-1 block w-full" :value="old('session', $user->session)" />
                <x-input-error class="mt-2" :messages="$errors->get('session')" />
            </div>

            <div>
                <x-input-label for="semester" :value="__('Semester')" />
                <x-text-input id="semester" name="semester" type="number" class="mt-1 block w-full" :value="old('semester', $user->semester)" min="1" max="12" />
